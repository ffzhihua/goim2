...
func initTCP() {
	...
	log.Info("net.Dial(\"%s\") ", Conf.TCPAddr)
	if err != nil {
	...
	proto.SeqId = seqId
	proto.Body = []byte("JSESSID_i0tmir1ieu3roii93pvkjcqi70")
	...
	log.Debug("auth ok, proto: %s", proto)
	seqId++
	// writer
	go func() {
		proto1 := new(Proto)
	...
	for {
		...
		if proto.Operation == OP_HEARTBEAT_REPLY {
			...
			log.Debug("test_reply_body: %s", string(proto.Body))
		} else if proto.Operation == OP_SEND_SMS_REPLY {
			log.Debug("sms_reply_body: %s", string(proto.Body))
		}
	}
}
