...
type BucketOptions struct {
	ChannelSize   int   //每个bucket有固定的channel个数
	RoomSize      int   //每个bucket有固定的room个数
...
type Bucket struct {
	...
	chs      map[string]*Channel // map sub key to a channel 订阅的KEY 对应的channel
	boptions BucketOptions
	// room
	rooms       map[int32]*Room                // bucket room channels room集合，每个room有多个channel
