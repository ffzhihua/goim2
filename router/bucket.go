...
type Bucket struct {
	...
	sessions          map[int64]*Session        // userid->sessions   一个可能同时多处登陆 存在多个seq:servers
...
// counter incr or decr counter.增加一个用户或者减少一个用户
...
// Put put a channel according with user id.关联channel(session)到指定的用户id，comet id以及room id
